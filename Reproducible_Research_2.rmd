...
### Processing Data
The dataset needs some further processing before it may be used for analysis.
...
However, many of the 'event types' describe the same type of event, and only vary because of spelling, punctuation, and word order.
...
StormSum1 <- dataset[, lapply(.SD, sum, na.rm=TRUE), by=EVTYPE, 
                     .SDcols=c('FATALITIES', 'INJURIES', 'PropertyDamage', 'CropDamage')]
...
```{r}
barchart(EVTYPE ~ PropertyDamage, data=StormSum3, horizontal = TRUE)
